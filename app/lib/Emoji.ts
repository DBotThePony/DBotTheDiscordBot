...
const EMOJI_FUNCS = {
	is: (input: string) => {
		return input.match(EMOJI_REGEXP_IS) != null
	},

	any: (input: string) => {
		return input.match(EMOJI_REGEXP) != null
	},

	matchAs: (input: string) => {
		return input.match(EMOJI_REGEXP)
	},

	match: (input: string) => {
		const match = input.match(EMOJI_REGEXP_IS)

		if (match) {
			return './resource/emoji/' + map[match[1]] + '.png'
		}
	...
	gather: (input: string) => {
		const match = input.match(EMOJI_REGEXP_GLOBAL)

		if (match) {
			const list: string[] = []

			for (const emote in match) {
				list.push('./resource/emoji/' + map[emote] + '.png')
			}
